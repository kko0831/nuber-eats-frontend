...
driver를 그리는 것은 엄청나게 쉬움

왜냐하면 google-map-react가 Child Component API를 가지고 있기 때문임

우리는 map 내부에 어떤 component든 렌더링 해줄 수 있고, 그 component에 위도랑 경도만 주면 됨

그럼 해봄

우선 div를 만들어주고, div안에 driver를 나타내는 자동차를 넣어줌

className으로 height랑 width를 8로 하고, 배경은 흰색으로 하고 rounded-full을 해줌

그리고 여기 div에 위도랑 경도를 넣어줘야함

그럼 lat은 driverCoords.lat으로 해주고 lng도 driverCoords.lng를 해줌

그러면 typescript가 뭐라고 하는데 맞음

이것이 정상임

왜냐하면 lat이랑 lng는 div에 없음

이것은 google-map-react를 위한 것임
...
그럼 여기에 flex랑 justify-center, items-center를 해줌
...
좀 작은 것 같으니 text-lg로 약간 키워줌

이것이 우리의 driver임

이제 내가 하고 싶은 것은 내가 위치를 바꾸거나 driver가 위치를 바꾸게 되면 map이 제대로 움직이는지 보고 싶음

그럼 한번 봄

우클릭해서 inspect를 들어가면 여기 보면 아직 보이지 않는데, 여기 보면 More tools라고 있음

여기서 Sensors라는 것을 클릭해줌

창을 좀 더 키워봄

Sensors는 내가 어디에 있든 Sensors를 재정의 해줄 수 있음

이 경우에는 베를린(Berlin)으로 해줌

여기 보면 내가 베를린으로 바꾸자마자 내 driver가 사라졌음

이 말은 여기 있는 watchPosition이 동작한다는 말임

왜냐하면 내 driver가 사라졌음

그런데 내 map은 그대로임

그래서 우리가 해줄 일은 map이랑 maps 객체를 state에 저장해줘야함

그래야 다양한 곳에서 접근이 가능해짐

이제 이 2개를 저장해줌

여기서 setMap(map)을 해주고 setMaps(maps)를 해줌

이제 우리는 이것들에 어디서든지 접근할 수 있음

그리고 driver가 움직일때마다 지도를 같이 움직여주고 싶음

그래서 또 다른 useEffect를 쓸건데 이번에는 driver의 좌표가 바뀔 때마다 실행되는 useEffect를 만듦

map.panTo는 이것과 똑같이 해주면 됨

이제 driver가 움직일때마다 이것이 호출됨

물론 map이랑 maps가 존재할때 그렇게 해야함

map이랑 maps가 null일 수도 있음

이제 우리는 driver가 어디든지 따라다님

새로고침 해보면 독일임

약간 조정을 해서 움직일때 어떻게 되는지 봄

내가 조금씩 움직일 때마다 지도도 같이 움직임
...
집에 있어서 어떻게 움직이는지 보여주기 힘들지만, 여기 보면 우리가 이것을 움직이면 이게 움직임

잘 동작함

키보드로 하니까 힘듦

이 말은 driver가 움직이면 우리도 움직임

그럼 상파울로(Sao Paulo)로 가봄

driver가 움직이면 지도도 움직이고, 마커는 항상 여기 있음

다시 한번 살짝 움직여보면 잘 동작됨

우리는 자동차랑 같이 움직이고 있음

여기까지가 step1임

step1이 끝났고 차가 움직이고 있음

만약 차가 이 길을 따라 움직이면, 우리도 같이 움직임

GPS를 따라감

다음으로는 subscription을 만들어줘야함

이 지도는 다 괜찮고 좋음

그런데 실제로 주문이 들어오는지 확인을 해줄 필요가 있음

그래서 그 전에 95vh를 50vh로 바꿔서 지도를 작게 만들어줌

zoom도 16으로 해줌

이제 약간 더 디테일해졌음

이제 새로운 주문이 들어오는지 알기 위해 subscription을 만듦
...
정말 많은 영상과 코드였지만 우리가 배운 것들에 정말 기쁨

백엔드(back-end)에서 Nest.JS를 배웠음

우리는 정말 섹시한 백엔드 구조를 가지고 있어서, 그냥 내가 원하는 어떤 것이든 안에 추가하면 됨

왜냐하면 이것이 정말 아름답게 동작하기 때문임

보스처럼 모든 dto를 다룰 수 있고, resolver와 service의 차이를 알게 되었음

그리고 이것을 테스트하면서 어떻게 service를 unit test하는지 배웠고, 어떻게 resolver를 end-to-end test하는지 배웠음

Nest.JS로 어떻게 Global modules를 만드는지 배웠음

이제 Nest.JS가 정말 쉬워졌음

NestJS로 어떻게 설정하는지도 배워서 JWT 미들웨어인 JWT guard를 만들었음

graphQL을 이용해서 authentication도 했고, HTTP 트래픽도 인증했고, WebSocket 트래픽도 인증했음

이것은 대단한 기술들이었고 아주 막대한 코딩이었음

이것들을 함께 만들어서 너무 기쁨

우리는 payment도 다뤘고 REST API로 업로드하는 것도 다뤄보면서, AWS로 파일을 업로드하기도 했음

정말 멋진 configuration object도 있음

나는 이것이 너무 좋음

그리고 프론트엔드 쪽도 아주 방대한 양의 작업이었음

3개의 대시보드를 만들었고 모두 다르게 보임

3개 모두 다른 요구사항을 가지고 있고, 모두 아름답게 작동했음

Google Maps을 다루는 법도 배웠고, 지도에서 어떻게 움직이게 하는지도 배웠음

유저를 어떻게 따라가는지도 배웠고, 포인트 A지점과 B지점 사이의 경로를 만드는 방법도 배웠음

여기서는 어떻게 component를 test하는지 배웠고, component를 unit test하는 방법도 배웠음

그리고 우리는 Cypress를 이용해서 end-to-end 테스트하는 방법도 배웠음

거기다가 Apollo의 캐시를 다루는 법도 배웠음

그리고 이메일을 인증하는 방법과 request를 보내지 않고 app 안에서 즉시 캐시를 변경하는 방법도 배웠음

아주 대단한 최적화 과정이었음

레스토랑 주인이 보는 페이지에서는 캐시에 대해 더 많은 것들을 배웠음

레스토랑을 생성하고 다시 첫 페이지로 돌아왔을때 생성한 레스토랑을 볼 수 있었음

아주 쿨한 기능임

Apollo 캐시에 대해 더 많이 배우게 됐음

여기 보이는 그래프를 다루는 방법도 배웠음

이것이 우리 웹사이트에 멋진 기능을 제공해줌

그리고 Client 페이지에서 만든 것들이 너무 자랑스러움

Typescript 덕분에 작동이 잘되고 문제가 없는 굉장히 좋은 주문 시스템을 만들었음

우리가 만든 것들에 대해 너무 행복함

정말 많은 것을 만들었는데 주문을 하고, 주문과 관련된 모든 사람들에게 알림을 보내고, 주문을 confirm 할 수도 있음

보기만 해도 정말 좋음

한번 더 봐야겠음

현재는 Pending이고 Accept Order하면 cooked로 바뀌었음

마치 사장님처럼 실시간 정보를 볼 수 있음

Order Cooked하면 이제 여기에 Cooked인 주문이 나타났고 배달 요청을 받았음

모든 창이 동기화 됐음

PickedUp하고 Delivered하면 이제 주문이 완료되서 "Nuber Eats를 이용해주셔서 감사합니다." 문구가 나옴
...
유저에게 주소를 물어볼 수 있다면 정말 어메이징함

이제 좌표를 주소로 바꾸는 법과 그 주소를 좌표로 변경하는 방법을 알고 있음

다음 섹션에서 다시 만날텐데 우리가 만든 것들을 deploy 해봄

프론트엔드는 netlify에 deploy하고, 백엔드는 Heroku에 deploy함
